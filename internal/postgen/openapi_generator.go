...
func replaceNodes(f *dst.File, genHf, currentHf HandlerFile, tag string) {
...
func (o *OpenapiGenerator) generateMergedFiles(handlers map[string]map[string]HandlerFile) error {
	// generate operation ids as typed constants for every tag. This way we get
	// a compile error if the spec doesn't use unique op ids and intellisense for middlewares, etc.
	s := path.Join(o.conf.OutHandlersDir, "operation_ids.gen.go")
	f, err := os.Create(s)
	if err != nil {
		return err
	}

	// operation ids indexed by tag.
	// TODO use Tag type
	operations := make(map[string][]string)
	dirs := []string{o.conf.GenHandlersDir}
	for _, dir := range dirs {
		for tag, hf := range handlers[dir] {
			opIDs := []string{}
			for op := range hf.Routes {
				opIDs = append(opIDs, op)
			}
			t := cases.Lower(language.English).String(tag)
			currentOps := append(operations[t], opIDs...)
			currentOps = removeDuplicates(currentOps)
			sort.Slice(currentOps, func(i, j int) bool {
				return currentOps[i] < currentOps[j]
			})
			operations[t] = currentOps
		}
	}
	...
	for tag, currentHF := range handlers[o.conf.CurrentHandlersDir] {
		...
		for _, gk := range gkk {
			...
			replaceNodes(outF, genHF, currentHF, tag)
		}
...
// inspectRegisterNode extracts the routes slice assignment node.
