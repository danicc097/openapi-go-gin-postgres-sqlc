...
(case when $1::boolean = true and _user_api_keys_user_ids.user_id is not null then row(_user_api_keys_user_ids.*) end) as user_api_key_user_id ` +
		`FROM xo_tests.user_api_keys ` +
		`-- O2O join generated from "user_api_keys_user_id_fkey(O2O inferred)"
left join xo_tests.user_api_keys as _user_api_keys_user_ids on _user_api_keys_user_ids.user_id = user_api_keys.user_id` +
