...
func (pe *PagElement) Insert(ctx context.Context, db DB) (*PagElement, error) {
	...
	rows, err := db.Query(ctx, sqlstr, pe.Name)
	if err != nil {
...
func (pe *PagElement) Update(ctx context.Context, db DB) (*PagElement, error) {
	...
	sqlstr := `UPDATE xo_tests.pag_element SET ` +
		`name = $1 ` +
		`WHERE paginated_element_id = $2 ` +
		`RETURNING * `
	// run
	logf(sqlstr, pe.Name, pe.CreatedAt, pe.PaginatedElementID)

	rows, err := db.Query(ctx, sqlstr, pe.Name, pe.PaginatedElementID)
	if err != nil {
