...
left join (
    ...
    group by
			book_sellers_seller
			, books.book_id
  ) as joined_books on joined_books.book_sellers_seller = book_sellers.seller
` +
...
book_sellers.seller, book_sellers.book_id, book_sellers.seller `
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.Sellers, c.joins.Books, bookID, seller)
	if err != nil {
...
book_sellers.seller, book_sellers.book_id, book_sellers.seller `
	...
	// run
	// logf(sqlstr, bookID)
	rows, err := db.Query(ctx, sqlstr, c.joins.Sellers, c.joins.Books, bookID)
	if err != nil {
...
book_sellers.seller, book_sellers.book_id, book_sellers.seller `
	...
	// run
	// logf(sqlstr, seller)
	rows, err := db.Query(ctx, sqlstr, c.joins.Sellers, c.joins.Books, seller)
	if err != nil {
		return nil, logerror(fmt.Errorf("BookSeller/BookSellerByBookIDSeller/Query: %w", err))
	}
	defer rows.Close()
