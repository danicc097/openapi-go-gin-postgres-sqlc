...
create table xo_tests.work_item_assignee (
  work_item_id bigint
  , assignee uuid
  ...
  , foreign key (assignee) references xo_tests.users (user_id)
);

create index on xo_tests.work_item_assignee (assignee , work_item_id);

comment on column xo_tests.work_item_assignee.work_item_id is '"cardinality":M2M';
...
begin
  ...
  insert into xo_tests.demo_work_items (work_item_id , checked)
    values (3 , true);
  insert into xo_tests.work_item_assignee (assignee , work_item_id , role)
