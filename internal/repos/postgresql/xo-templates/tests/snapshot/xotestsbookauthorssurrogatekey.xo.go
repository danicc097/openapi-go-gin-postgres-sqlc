	...
	if err != nil {
		...
		if errors.As(err, &pgErr) {
			if pgErr.Code != pgerrcode.UniqueViolation {
				return nil, fmt.Errorf("UpsertXoTestsBookAuthorsSurrogateKey/Insert: %w", &XoError{Entity: "Book authors surrogate key", Err: err})
			}
			xtbask, err = xtbask.Update(ctx, db)
			if err != nil {
				return nil, fmt.Errorf("UpsertXoTestsBookAuthorsSurrogateKey/Update: %w", &XoError{Entity: "Book authors surrogate key", Err: err})
			}
		}
	}
...
func XoTestsBookAuthorsSurrogateKeyPaginated(ctx context.Context, db DB, cursor models.PaginationCursor, opts ...XoTestsBookAuthorsSurrogateKeySelectConfigOption) ([]XoTestsBookAuthorsSurrogateKey, error) {
	...
	if len(c.orderBy) > 0 {
		...
		return nil, logerror(fmt.Errorf("XoTestsBookAuthorsSurrogateKey/Paginated/orderBy: %w", &XoError{Entity: "Book authors surrogate key", Err: fmt.Errorf("at least one sorted column is required")}))
	}
	i := 0
	...
	sqlstr += c.limit
	sqlstr = "/* XoTestsBookAuthorsSurrogateKeyPaginated */\n" + sqlstr
