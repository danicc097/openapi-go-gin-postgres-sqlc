...
(case when $1::boolean = true and _user_api_key_ids.api_key_id is not null then row(_user_api_key_ids.*) end) as user_user_api_key_id ` +
		`FROM xo_tests.user_api_keys ` +
		`-- O2O join generated from "users_api_key_id_fkey(O2O inferred)"
left join xo_tests.users as _user_api_key_ids on _user_api_key_ids.api_key_id = user_api_keys.user_api_key_id` +
		` WHERE user_api_keys.user_api_key_id > $2 GROUP BY _user_api_key_ids.api_key_id,
      _user_api_key_ids.user_id,
	user_api_keys.user_api_key_id `
	sqlstr += c.limit
...
(case when $1::boolean = true and _user_api_key_ids.api_key_id is not null then row(_user_api_key_ids.*) end) as user_user_api_key_id ` +
		...
		`-- O2O join generated from "users_api_key_id_fkey(O2O inferred)"
left join xo_tests.users as _user_api_key_ids on _user_api_key_ids.api_key_id = user_api_keys.user_api_key_id` +
		` WHERE user_api_keys.api_key = $2 GROUP BY _user_api_key_ids.api_key_id,
      _user_api_key_ids.user_id,
	user_api_keys.user_api_key_id `
	sqlstr += c.orderBy
	sqlstr += c.limit
...
(case when $1::boolean = true and _user_api_key_ids.api_key_id is not null then row(_user_api_key_ids.*) end) as user_user_api_key_id ` +
		...
		`-- O2O join generated from "users_api_key_id_fkey(O2O inferred)"
left join xo_tests.users as _user_api_key_ids on _user_api_key_ids.api_key_id = user_api_keys.user_api_key_id` +
		` WHERE user_api_keys.user_id = $2 GROUP BY _user_api_key_ids.api_key_id,
      _user_api_key_ids.user_id,
	user_api_keys.user_api_key_id `
	sqlstr += c.orderBy
	sqlstr += c.limit
