...
type User struct {
	...
	NotificationsJoin     *[]Notification     `json:"-" db:"notifications" openapi-go:"ignore"`      // O2M
	NotificationsJoin     *[]Notification     `json:"-" db:"notifications" openapi-go:"ignore"`      // O2M
...
users.updated_at,
users.deleted_at,
(case when $1::boolean = true then COALESCE(joined_notifications.notifications, '{}') end) as notifications,
(case when $2::boolean = true then COALESCE(joined_notifications.notifications, '{}') end) as notifications,
...
left join (
  ...
  from
    notifications
  group by
        receiver) joined_notifications on joined_notifications.notifications_user_id = users.user_id
...
left join (
  ...
  from
    notifications
  group by
        sender) joined_notifications on joined_notifications.notifications_user_id = users.user_id
...
`+
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.Notifications, c.joins.Notifications, c.joins.TimeEntries, c.joins.UserAPIKey, c.joins.UserNotifications, c.joins.Teams, c.joins.WorkItems, createdAt)
	if err != nil {
		return nil, logerror(err)
	}
	defer rows.Close()
...
`+
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.Notifications, c.joins.Notifications, c.joins.TimeEntries, c.joins.UserAPIKey, c.joins.UserNotifications, c.joins.Teams, c.joins.WorkItems, deletedAt)
	if err != nil {
		return nil, logerror(err)
	}
	defer rows.Close()
...
`+
	...
	// run
	// logf(sqlstr, email)
	rows, err := db.Query(ctx, sqlstr, c.joins.Notifications, c.joins.Notifications, c.joins.TimeEntries, c.joins.UserAPIKey, c.joins.UserNotifications, c.joins.Teams, c.joins.WorkItems, email)
	if err != nil {
...
`+
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.Notifications, c.joins.Notifications, c.joins.TimeEntries, c.joins.UserAPIKey, c.joins.UserNotifications, c.joins.Teams, c.joins.WorkItems, externalID)
	if err != nil {
...
`+
	...
	// run
	// logf(sqlstr, userID)
	rows, err := db.Query(ctx, sqlstr, c.joins.Notifications, c.joins.Notifications, c.joins.TimeEntries, c.joins.UserAPIKey, c.joins.UserNotifications, c.joins.Teams, c.joins.WorkItems, userID)
	if err != nil {
...
`+
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.Notifications, c.joins.Notifications, c.joins.TimeEntries, c.joins.UserAPIKey, c.joins.UserNotifications, c.joins.Teams, c.joins.WorkItems, updatedAt)
	if err != nil {
		return nil, logerror(err)
	}
	defer rows.Close()
...
`+
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.Notifications, c.joins.Notifications, c.joins.TimeEntries, c.joins.UserAPIKey, c.joins.UserNotifications, c.joins.Teams, c.joins.WorkItems, username)
	if err != nil {
