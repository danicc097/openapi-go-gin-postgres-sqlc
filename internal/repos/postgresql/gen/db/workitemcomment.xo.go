...
func WorkItemCommentPaginatedByWorkItemCommentID(ctx context.Context, db DB, , opts ...WorkItemCommentSelectConfigOption) ([]WorkItemComment, error) {
...
left join work_items on work_items.work_item_id = work_item_comments.work_item_id` +
	...
	rows, err := db.Query(ctx, sqlstr, wic.WorkItemCommentID)
	if err != nil {
