...
(case when $2::boolean = true then row(users.*) end) as user,
(case when $3::boolean = true then row(user_notifications.*) end) as user_notification ` +
...
left join user_notifications on user_notifications.notification_id = notifications.notification_id` +
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.User, c.joins.User, c.joins.UserNotification, notificationID)
	if err != nil {
...
left join user_notifications on user_notifications.notification_id = notifications.notification_id` +
	...
	rows, err := db.Query(ctx, sqlstr, c.joins.User, c.joins.User, c.joins.UserNotification, receiverRank, notificationType, createdAt)
	if err != nil {
