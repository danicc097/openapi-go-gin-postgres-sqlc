...
func TimeEntryPaginatedByTimeEntryID(ctx context.Context, db DB, , opts ...TimeEntrySelectConfigOption) ([]TimeEntry, error) {
...
left join work_items on work_items.work_item_id = time_entries.work_item_id` +
	...
	rows, err := db.Query(ctx, sqlstr, te.TimeEntryID)
	if err != nil {
